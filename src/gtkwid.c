...
void x_init(Display *display, Window childWindowId, Window parentWindowId) {
  // We need to move the child window before reparenting it to avoid some nasty
  // offsets
  ...
  // client_event.xconfigure.x = parentAttributes.x +
  // windowDecorationSize.width; client_event.xconfigure.y = parentAttributes.y
  // + windowDecorationSize.height;
